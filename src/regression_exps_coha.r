...
option_list = list(
  ...
  make_option(c("-y", "--year"), type="character", default="10,20,50,100,10000")
); 
...
rsquared_df<-bind_rows(list_of_rsqr)
rsquared_df$cutoff<-as.factor(rsquared_df$cutoff)
write.csv(rsquared_df,paste0("/fs/scratch/users/pageljs/compounding/coha/regression/rsquared_coha_",opt$ppmi,"_",opt$setting,".csv"),row.names = FALSE)


varimp_10000_df<-bind_rows(list_of_vi_10000)
varimp_10000_df$cutoff<-as.factor(varimp_10000_df$cutoff)
varimp_10000_df[is.na(varimp_10000_df)] <- 0
write.csv(varimp_10000_df,"/fs/scratch/users/pageljs/compounding/coha/regression/varimp_10000_coha_",opt$ppmi,"_",opt$setting,".csv",row.names = FALSE)

varimp_10_df<-bind_rows(list_of_vi_10)
varimp_10_df$cutoff<-as.factor(varimp_10_df$cutoff)
varimp_10_df[is.na(varimp_10_df)] <- 0
write.csv(varimp_10_df,"/fs/scratch/users/pageljs/compounding/coha/regression/varimp_10_coha_",opt$ppmi,"_",opt$setting,".csv",row.names = FALSE)


varimp_20_df<-bind_rows(list_of_vi_20)
varimp_20_df$cutoff<-as.factor(varimp_20_df$cutoff)
varimp_20_df[is.na(varimp_20_df)] <- 0
write.csv(varimp_20_df,"/fs/scratch/users/pageljs/compounding/coha/regression/varimp_20_coha_",opt$ppmi,"_",opt$setting,".csv",row.names = FALSE)

varimp_50_df<-bind_rows(list_of_vi_50)
varimp_50_df$cutoff<-as.factor(varimp_50_df$cutoff)
varimp_50_df[is.na(varimp_20_df)] <- 0
write.csv(varimp_50_df,"/fs/scratch/users/pageljs/compounding/coha/regression/varimp_50_coha_",opt$ppmi,"_",opt$setting,".csv",row.names = FALSE)


varimp_100_df<-bind_rows(list_of_vi_100)
varimp_100_df$cutoff<-as.factor(varimp_100_df$cutoff)
varimp_100_df[is.na(varimp_100_df)] <- 0
write.csv(varimp_100_df,paste0("/fs/scratch/users/pageljs/compounding/coha/regression/varimp_100_coha_",opt$ppmi,"_",opt$setting,".csv"),row.names = FALSE)
